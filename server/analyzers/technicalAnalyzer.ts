...
export class TechnicalAnalyzer {
  public static analyze(
  ...
  ): { summary: string; findings: AuditFinding[]; httpsStatus: string; redirectsStatus: string; brokenLinksCount: number; brokenImagesCount: number; consoleErrorsCount: number } {
    ...
    if (homePage) {
      ...
      if (!headers.hsts) missingHeaders.push('Strict-Transport-Security (HSTS)');
      if (!headers.csp) missingHeaders.push('Content-Security-Policy (CSP)');
      if (!headers.xFrameOptions) missingHeaders.push('X-Frame-Options');
      if (!headers.xContentTypeOptions) missingHeaders.push('X-Content-Type-Options');
