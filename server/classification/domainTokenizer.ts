...
export class DomainTokenizer {
   ...
   */
  public static tokenize(url: string): { tokens: string[]; rawHost: string; baseName: string } {
    let rawHost = '';
    try {
   ...
   */
  public static segmentCompoundWord(word: string): string[] {
    ...
    // dp[i] stores the best segmentation score and token list for substring lower.slice(0, i)
    interface DPState {
      score: number;
      tokens: string[];
    }

    const dp: Array<DPState | null> = new Array(n + 1).fill(null);
    dp[0] = { score: 0, tokens: [] };
   ...
   */
  public static filterNoise(tokens: string[]): string[] {
   ...
   */
  public static formatTitleFromDomain(url: string): string {
