...
import { fetchAndParseRobotsTxt, isPathDisallowed } from './robotsParser.js';
import { EvidenceRegistry } from '../evidence/evidenceRegistry.js';
import { PageRecord } from '../types/audit.js';
import { CONFIG } from '../config.js';
import { validateUrlForSSRF } from '../security/ssrfGuard.js';

export interface CrawlerCallbacks {
  onProgress?: (progressPercent: number, message: string, pageUrl?: string) => void;
  isCancelled?: () => boolean;
}
...
export class CrawlerEngine {
  ...
  public static async crawl(
    ...
    maxPages: number = CONFIG.MAX_PAGES_DEFAULT,
    ...
    const ssrfCheck = await validateUrlForSSRF(rootUrl);
    ...
    const robotsPolicy = await fetchAndParseRobotsTxt(normalizedRoot);
    ...
    try {
      context = await BrowserPool.createSecureContext();
      ...
      while (urlQueue.length > 0 && visitedUrls.size < maxPages) {
        ...
        if (isPathDisallowed(currentParsed.pathname, robotsPolicy.disallowedPaths)) {
          evidenceRegistry.register({
        ...
        callbacks?.onProgress?.(
          progressPercent,
          `Crawling page ${pageNum}/${Math.min(urlQueue.length + pageNum, maxPages)}: ${normalizedCurrent}`,
          normalizedCurrent
        );

        const pageRecord = await this.inspectSinglePage(
          context,
          normalizedCurrent,
          auditId,
          evidenceRegistry,
          pageNum === 1 // isHomePage
        );
          ...
          const newInternalLinks = pageRecord.internalLinks
            ...
            .filter((l) => {
              try {
                const p = new URL(l);
                return (
                  this.isSameDomain(p.hostname, rootHostname) &&
                  !visitedUrls.has(l) &&
                  !urlQueue.includes(l) &&
   ...
   */
  private static async inspectSinglePage(
    ...
    const networkErrors: Array<{ url: string; status: number | string; statusText?: string }> = [];
    ...
    let desktopScreenshotRel: string | undefined;
    let mobileScreenshotRel: string | undefined;

    if (isHomePage || statusCode === 200) {
      try {
        const prefix = isHomePage ? 'homepage' : `page-${Date.now()}`;
        const desk = await ScreenshotManager.captureDesktop(page, auditId, `${prefix}-desktop`);
        desktopScreenshotRel = desk.relPath;

        const mob = await ScreenshotManager.captureMobile(page, auditId, `${prefix}-mobile`);
    ...
    return {
      ...
      securityHeaders: securityHeaders as any,
  ...
  private static isAssetOrFile(pathname: string): boolean {
  ...
  private static getPriorityScore(urlStr: string): number {
  ...
  private static isSameDomain(hostA: string, hostB: string): boolean {
