...
class GeminiProvider {
    ...
    client = null;
    constructor() {
        if (config_js_1.CONFIG.GEMINI_API_KEY) {
            this.client = new genai_1.GoogleGenAI({ apiKey: config_js_1.CONFIG.GEMINI_API_KEY });
        }
    }
    isAvailable() {
        return !!config_js_1.CONFIG.GEMINI_API_KEY;
    }
  ...
  "nextSteps": [
    {
      ...
      "timelineRecommendation": "Week 1"
        ...
        if (input.desktopScreenshotPath) {
            try {
                const imageBuffer = await promises_1.default.readFile(input.desktopScreenshotPath);
                contents.push({
                    inlineData: {
                        mimeType: 'image/png',
