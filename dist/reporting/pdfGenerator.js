...
const browserPool_js_1 = require("../crawler/browserPool.js");
const config_js_1 = require("../config.js");
const sanitizer_js_1 = require("../security/sanitizer.js");
const salesReportSynthesizer_js_1 = require("./salesReportSynthesizer.js");
class PdfGenerator {
    /**
     * Generates a professional branded PDF report for an audit job in Client, Internal, or Sales mode.
     */
    static async generate(audit, mode = 'client') {
        const report = audit.report;
        if (!report && mode !== 'sales') {
            throw new Error('Cannot generate PDF: Audit report data is missing');
        }
        const pdfDir = config_js_1.CONFIG.PDFS_DIR;
        await promises_1.default.mkdir(pdfDir, { recursive: true });
        ...
        if (homePage?.desktopScreenshot) {
            try {
                const fullPath = path_1.default.join(config_js_1.CONFIG.DATA_DIR, homePage.desktopScreenshot.replace(/^\//, ''));
        ...
        if (homePage?.mobileScreenshot) {
            try {
                const fullPath = path_1.default.join(config_js_1.CONFIG.DATA_DIR, homePage.mobileScreenshot.replace(/^\//, ''));
                const buffer = await promises_1.default.readFile(fullPath);
        ...
        const context = await browserPool_js_1.BrowserPool.createSecureContext();
     ...
     */
    static buildClientReportHtml(audit, desktopImg, mobileImg) {
        const report = audit.report;
      ...
      <div class="card" style="margin-bottom: 12px; border-left: 4px solid #38BDF8;">
        <h4 style="font-size: 14px; font-weight: 700; color: #0F172A; margin-bottom: 4px;">${(0, sanitizer_js_1.escapeHtml)(opp.title)}</h4>
        <p style="font-size: 12px; color: #475569; margin-bottom: 6px;"><strong>Business Impact:</strong> ${(0, sanitizer_js_1.escapeHtml)(opp.businessImpact)}</p>
        <p style="font-size: 12px; color: #0284C7; font-weight: 600;"><strong>Recommended Action:</strong> ${(0, sanitizer_js_1.escapeHtml)(opp.recommendedAction)}</p>
      </div>
    `)
            .join('');
        const strengthsHtml = (clientSum?.currentStrengths || report.executiveSummary.topStrengths || [])
            .map((str) => `<li style="margin-bottom: 6px;">${(0, sanitizer_js_1.escapeHtml)(str)}</li>`)
            .join('');
        const structureHtml = (clientSum?.recommendedWebsiteStructure || report.recommendedWebsiteStructure.suggestedSitemap || [])
            .map((s) => `
      <div class="card" style="margin-bottom: 10px;">
        <div style="font-weight: 700; font-size: 13px; color: #0F172A;">${(0, sanitizer_js_1.escapeHtml)(s.pageName)}</div>
        <div style="font-size: 12px; color: #475569; margin: 3px 0;">${(0, sanitizer_js_1.escapeHtml)(s.purpose)}</div>
        ${s.keySections ? `<div style="font-size: 11px; color: #64748B;"><strong>Recommended Sections:</strong> ${s.keySections.map((k) => (0, sanitizer_js_1.escapeHtml)(k)).join(' • ')}</div>` : ''}
      </div>
    `)
            .join('');
      ...
      <div style="display: flex; gap: 12px; align-items: flex-start; padding: 10px; background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; margin-bottom: 8px;">
        ...
        <div style="flex: 1;">
          <div style="font-size: 12px; font-weight: 700; color: #0F172A;">${(0, sanitizer_js_1.escapeHtml)(step.step)}</div>
          <div style="font-size: 11px; color: #0284C7; font-weight: 600; margin-top: 2px;">Target Timeline: ${(0, sanitizer_js_1.escapeHtml)(step.timeframe)}</div>
        </div>
      </div>
    `)
            .join('');
...
<head>
  <meta charset="UTF-8">
  <title>Website Strategic Report - ${(0, sanitizer_js_1.escapeHtml)(audit.config.url)}</title>
  <style>
...
<body>
  ...
  <div style="padding: 48px 36px; min-height: 85vh; display: flex; flex-direction: column; justify-content: space-between; border-radius: 12px; background: linear-gradient(135deg, #0F172A 0%, #1E293B 100%); color: #FFF;">
    ...
    <div style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.12); padding: 18px; border-radius: 8px; display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
      <div><span style="font-size: 9px; color: #94A3B8; text-transform: uppercase; display: block; font-weight: 700;">Target Website</span><strong style="font-size: 13px; color: #F1F5F9; word-break: break-all;">${(0, sanitizer_js_1.escapeHtml)(audit.config.url)}</strong></div>
      ...
      <div><span style="font-size: 9px; color: #94A3B8; text-transform: uppercase; display: block; font-weight: 700;">Business Name</span><strong style="font-size: 13px; color: #F1F5F9;">${(0, sanitizer_js_1.escapeHtml)(audit.config.businessName || 'Business name not provided')}</strong></div>
      <div><span style="font-size: 9px; color: #94A3B8; text-transform: uppercase; display: block; font-weight: 700;">Target Location</span><strong style="font-size: 13px; color: #F1F5F9;">${(0, sanitizer_js_1.escapeHtml)(audit.config.location || 'Target location was not provided.')}</strong></div>
    </div>
  </div>
  ...
  <div style="padding: 10px 0;">
    ...
    <div class="card" style="margin-bottom: 14px; background: #F0F9FF; border-color: #BAE6FD;">
      <p style="font-size: 12px; color: #0369A1; font-weight: 500; line-height: 1.6;">${(0, sanitizer_js_1.escapeHtml)(clientSum?.overview || report.executiveSummary.summary)}</p>
    </div>
  ...
  <div style="text-align: center; font-size: 9.5px; color: #94A3B8; margin-top: 24px; border-top: 1px solid #E2E8F0; padding-top: 10px;">
     ...
     */
    static buildInternalReportHtml(audit, desktopImg, mobileImg) {
        const report = audit.report;
        ...
        <div class="finding-card">
          <div style="display: flex; gap: 6px; margin-bottom: 6px;">
            <span style="background: ${color}; color: #FFF; padding: 2px 6px; border-radius: 4px; font-size: 9px; font-weight: bold;">${(0, sanitizer_js_1.escapeHtml)(f.priority)}</span>
            <span style="background: #E2E8F0; color: #334155; padding: 2px 6px; border-radius: 4px; font-size: 9px; font-weight: bold;">${(0, sanitizer_js_1.escapeHtml)(f.verificationStatus)}</span>
            <span style="background: #0F172A; color: #38BDF8; padding: 2px 6px; border-radius: 4px; font-size: 9px; font-family: monospace;">${(0, sanitizer_js_1.escapeHtml)(f.category.toUpperCase())}</span>
            ${f.evidenceIds?.length ? `<span style="color: #0284C7; font-size: 9px; font-family: monospace;">${f.evidenceIds.join(', ')}</span>` : ''}
          </div>
          <h4 style="font-size: 13px; font-weight: 700; color: #0F172A; margin-bottom: 4px;">${(0, sanitizer_js_1.escapeHtml)(f.title)}</h4>
          <p style="font-size: 11px; color: #334155; margin-bottom: 6px;"><strong>Observation:</strong> ${(0, sanitizer_js_1.escapeHtml)(f.observation)}</p>
          ${f.evidence ? `<div style="background: #F1F5F9; padding: 6px; border-radius: 4px; font-family: monospace; font-size: 10px; color: #475569; margin-bottom: 6px;"><strong>Evidence:</strong> ${(0, sanitizer_js_1.escapeHtml)(f.evidence)}</div>` : ''}
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 10px; background: #F8FAFC; padding: 8px; border-radius: 4px;">
            <div><strong>Impact:</strong> ${(0, sanitizer_js_1.escapeHtml)(f.impact)}</div>
            <div><strong>Recommendation:</strong> ${(0, sanitizer_js_1.escapeHtml)(f.recommendation)}</div>
          </div>
        </div>
      `;
        })
      ...
      <tr>
        ...
        <td style="max-width: 200px; word-break: break-all; font-family: monospace; font-size: 10px;">${(0, sanitizer_js_1.escapeHtml)(p.url)}</td>
        <td>${(0, sanitizer_js_1.escapeHtml)(p.title || 'Untitled')}</td>
...
<head>
  <meta charset="UTF-8">
  <title>Internal Technical Audit - ${(0, sanitizer_js_1.escapeHtml)(audit.config.url)}</title>
  <style>
...
<body>
  ...
  <div style="padding: 20px; background: #0F172A; color: #FFF; border-radius: 8px; margin-bottom: 20px;">
    ...
    <div style="font-size: 11px; color: #94A3B8;">Target URL: ${(0, sanitizer_js_1.escapeHtml)(audit.config.url)} • Audit ID: ${audit.id} • Date: ${dateStr}</div>
  </div>
  ...
  <div style="text-align: center; font-size: 10px; color: #94A3B8; margin-top: 30px; border-top: 1px solid #E2E8F0; padding-top: 12px;">
     ...
     */
    static buildSalesReportHtml(audit, desktopImg, mobileImg) {
        const sales = audit.salesReport || salesReportSynthesizer_js_1.SalesReportSynthesizer.synthesize(audit);
        const dateStr = new Date(audit.createdAt).toLocaleDateString('en-US', {
            year: 'numeric',
            month: 'long',
            day: 'numeric',
      ...
      <div class="card" style="margin-bottom: 8px; border-left: 4px solid #10B981; background: #F0FDF4;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 3px;">
          <strong style="font-size: 12px; color: #065F46;">${(0, sanitizer_js_1.escapeHtml)(w.title)}</strong>
          <span style="font-size: 9px; font-weight: bold; background: #DCFCE7; color: #166534; padding: 2px 6px; border-radius: 4px;">${(0, sanitizer_js_1.escapeHtml)(w.status)} • ${w.evidenceIds.join(', ')}</span>
        </div>
        <p style="font-size: 11px; color: #15803D; line-height: 1.4;">${(0, sanitizer_js_1.escapeHtml)(w.explanation)}</p>
      </div>
    `)
            .join('');
        ...
        <div class="card" style="margin-bottom: 10px; border-left: 4px solid ${style.border};">
          <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 4px;">
            <h4 style="font-size: 13px; font-weight: 700; color: #0F172A;">${idx + 1}. ${(0, sanitizer_js_1.escapeHtml)(opp.title)}</h4>
            <div style="display: flex; gap: 4px;">
              <span style="background: ${style.bg}; color: ${style.text}; font-size: 8.5px; font-weight: bold; padding: 2px 6px; border-radius: 4px; text-transform: uppercase;">${(0, sanitizer_js_1.escapeHtml)(opp.priority)}</span>
          ...
          <p style="font-size: 11px; color: #475569; margin-bottom: 4px;"><strong>Current Observation:</strong> ${(0, sanitizer_js_1.escapeHtml)(opp.clientObservation)}</p>
          <p style="font-size: 11px; color: #0F172A; margin-bottom: 4px;"><strong>Business Impact:</strong> ${(0, sanitizer_js_1.escapeHtml)(opp.businessImpact)}</p>
          <div style="background: #F0F9FF; border: 1px solid #BAE6FD; padding: 6px 10px; border-radius: 4px; font-size: 11px; color: #0284C7; font-weight: 600;">
            👉 <strong>Recommended Action:</strong> ${(0, sanitizer_js_1.escapeHtml)(opp.recommendedAction)}
          </div>
        </div>
      `;
        })
      ...
      <div class="card" style="margin-bottom: 8px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2px;">
          <strong style="font-size: 12px; color: #0F172A;">📄 ${(0, sanitizer_js_1.escapeHtml)(s.pageName)}</strong>
        ...
        <p style="font-size: 11px; color: #475569; margin-bottom: 4px;">${(0, sanitizer_js_1.escapeHtml)(s.purpose)}</p>
        <div style="font-size: 10px; color: #64748B;"><strong>Key Conversion Sections:</strong> ${s.keySections.map((k) => (0, sanitizer_js_1.escapeHtml)(k)).join(' • ')}</div>
      </div>
    `)
            .join('');
      ...
      <div style="display: flex; gap: 10px; align-items: flex-start; padding: 8px 10px; background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 6px; margin-bottom: 6px;">
        ...
        <div style="flex: 1;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 2px;">
            <strong style="font-size: 11.5px; color: #0F172A;">${(0, sanitizer_js_1.escapeHtml)(r.title)}</strong>
            <span style="font-size: 10px; color: #0284C7; font-weight: 600;">${(0, sanitizer_js_1.escapeHtml)(r.timeframe)}</span>
          </div>
          <p style="font-size: 10.5px; color: #475569;">${(0, sanitizer_js_1.escapeHtml)(r.description)}</p>
        </div>
      </div>
    `)
            .join('');
        const displayBizNameHtml = sales.businessNameProvenance === 'USER_PROVIDED'
            ? (0, sanitizer_js_1.escapeHtml)(sales.businessName)
            : sales.businessNameProvenance === 'INFERRED FROM DOMAIN'
                ? `${(0, sanitizer_js_1.escapeHtml)(sales.businessName)} <span style="font-size: 8.5px; color: #38BDF8; font-weight: normal;">(INFERRED FROM DOMAIN)</span>`
                : sales.businessNameProvenance === 'INFERRED FROM WEBSITE'
                    ? `${(0, sanitizer_js_1.escapeHtml)(sales.businessName)} <span style="font-size: 8.5px; color: #38BDF8; font-weight: normal;">(INFERRED FROM WEBSITE)</span>`
                    : `<span style="color: #94A3B8; font-weight: normal;">Business name not provided</span>`;
        const displayLocationHtml = sales.locationProvenance !== 'NOT_PROVIDED'
            ? (0, sanitizer_js_1.escapeHtml)(sales.location)
            : `<span style="color: #94A3B8; font-weight: normal;">Target location was not provided.</span>`;
        const provenanceLabel = sales.classificationProvenance === 'USER_PROVIDED'
            ? 'USER PROVIDED'
        ...
        const industryDisplay = sales.businessType && sales.businessType !== sales.industry && sales.businessType !== 'Unknown Business Type'
...
<head>
  <meta charset="UTF-8">
  <title>Client Sales Audit - ${(0, sanitizer_js_1.escapeHtml)(audit.config.url)}</title>
  <style>
    @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&family=Inter:wght@400;500;600;700&display=swap');
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body {
...
<body>
  ...
  <div style="padding: 36px 28px; min-height: 85vh; display: flex; flex-direction: column; justify-content: space-between; border-radius: 10px; background: linear-gradient(135deg, #0B132B 0%, #1C2541 100%); color: #FFF;">
    <div>
      <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <span style="background: rgba(56, 189, 248, 0.15); color: #38BDF8; padding: 3px 8px; border-radius: 4px; font-size: 8.5px; font-weight: bold; border: 1px solid rgba(56, 189, 248, 0.3);">${(0, sanitizer_js_1.escapeHtml)(industryDisplay)} • ${(0, sanitizer_js_1.escapeHtml)(sales.industryConfidence)} • ${(0, sanitizer_js_1.escapeHtml)(provenanceLabel)}</span>
      </div>
    ...
    <div style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.12); padding: 14px 18px; border-radius: 8px; display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
      ...
      <div><span style="font-size: 9px; color: #94A3B8; text-transform: uppercase; display: block; font-weight: 700;">Website URL</span><strong style="font-size: 12px; color: #F1F5F9; word-break: break-all;">${(0, sanitizer_js_1.escapeHtml)(audit.config.url)}</strong></div>
    ...
    <div style="background: ${sales.isParkedOrIncomplete ? 'rgba(239, 68, 68, 0.12)' : 'rgba(16, 185, 129, 0.12)'}; border: 1px solid ${sales.isParkedOrIncomplete ? 'rgba(239, 68, 68, 0.3)' : 'rgba(16, 185, 129, 0.3)'}; padding: 10px 14px; border-radius: 6px;">
      ...
      <p style="font-size: 10.5px; color: #E2E8F0; line-height: 1.4;">${(0, sanitizer_js_1.escapeHtml)(sales.websiteStatusSummary)}</p>
    </div>
  </div>
  ...
  <div style="padding: 10px 0;">
    ...
    <div class="card" style="margin-bottom: 12px; background: #F8FAFC;">
      ...
      <p style="font-size: 10.5px; color: #475569; line-height: 1.4;">${(0, sanitizer_js_1.escapeHtml)(sales.whyTheseMatter)}</p>
    </div>

    <h3 style="font-size: 12px; font-weight: 700; color: #0F172A; margin: 10px 0 6px 0;">Recommended 5-Page Website Blueprint (${(0, sanitizer_js_1.escapeHtml)(sales.industry)})</h3>
    ${structureHtml}
    ...
    <div style="margin-top: 14px; background: linear-gradient(135deg, #0F172A 0%, #1E293B 100%); color: #FFF; padding: 14px; border-radius: 8px; border: 1px solid #38BDF8;">
      ...
      <h4 style="font-size: 13px; font-weight: 700; color: #F8FAFC; margin: 4px 0;">${(0, sanitizer_js_1.escapeHtml)(sales.agencyCta.title)}</h4>
      <p style="font-size: 10.5px; color: #CBD5E1; margin-bottom: 8px; line-height: 1.4;">${(0, sanitizer_js_1.escapeHtml)(sales.agencyCta.message)}</p>
      <div style="font-size: 10.5px; font-weight: 700; color: #38BDF8;">👉 ${(0, sanitizer_js_1.escapeHtml)(sales.agencyCta.contactPrompt)}</div>
    </div>
  </div>
