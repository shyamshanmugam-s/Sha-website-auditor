...
exports.IndustryClassifier = void 0;
const industryTaxonomy_js_1 = require("./industryTaxonomy.js");
const domainTokenizer_js_1 = require("./domainTokenizer.js");
class IndustryClassifier {
    ...
    static classify(audit) {
        ...
        if (config.industry && config.industry.trim().length > 0) {
            ...
            return {
                ...
                evidenceIds: (audit.crawlResult?.pages || audit.pages)?.[0]?.evidenceIds?.slice(0, 1) || ['E-001'],
        ...
        const pages = audit.crawlResult?.pages || audit.pages || [];
        ...
        const { tokens: rawDomainTokens } = domainTokenizer_js_1.DomainTokenizer.tokenize(config.url);
        const domainTokens = domainTokenizer_js_1.DomainTokenizer.filterNoise(rawDomainTokens);
        // Score all industries in the taxonomy
        const scores = [];
        for (const industry of industryTaxonomy_js_1.INDUSTRY_TAXONOMY) {
            if (industry.id === 'unknown' || industry.id === 'other')
                continue;
            for (const bType of industry.businessTypes) {
                const scoreResult = this.evaluateSubIndustry(industry, bType, pages, structuredSchemaTypes, domainTokens, audit, isParkedOrIncomplete);
        ...
        const bothContentEvidencePresent = !isParkedOrIncomplete &&
            runnerUp !== null &&
            top.hasContentMatch &&
        ...
        let confidence = 'LOW';
        let provenance = 'DOMAIN_TOKEN';
        let verificationStatus = 'REQUIRES CLIENT APPROVAL';
     ...
     */
    static evaluateSubIndustry(industry, bType, pages, structuredSchemaTypes, domainTokens, audit, isParked) {
        let score = 0;
        const evidenceIds = [];
        ...
        if (!isParked && pages.length > 0) {
            ...
            for (let pIdx = 0; pIdx < pages.length; pIdx++) {
                ...
                const navText = (page.links?.internal?.map((l) => l.text) || []).join(' ').toLowerCase();
                ...
                for (const kw of bType.keywords) {
                    if (this.containsKeyword(fullPageText, kw)) {
                        ...
                        if (this.containsKeyword(titleText, kw))
                            kwScore += 10;
                        if (this.containsKeyword(headingsText, kw))
                            kwScore += 8;
                        score += kwScore;
                        pageMatched = true;
                        ...
                        if (page.evidenceIds?.length)
                            evidenceIds.push(page.evidenceIds[0]);
                    }
                }
        ...
        for (const kw of bType.keywords) {
            ...
            if (kwWords.length === 1) {
                ...
                if (lowerTokens.includes(singleKw)) {
                    ...
                    if (pages[0]?.evidenceIds?.length)
                        evidenceIds.push(pages[0].evidenceIds[0]);
                }
            }
            else {
     ...
     */
    static containsKeyword(text, kw) {
     ...
     */
    static extractSchemaTypes(audit, pages) {
        const types = [];
        // From Crawled Pages
        for (const p of pages) {
            if (p.structuredData?.jsonLd && Array.isArray(p.structuredData.jsonLd)) {
                for (const item of p.structuredData.jsonLd) {
                    if (item?.['@type'])
                        types.push(String(item['@type']).toLowerCase());
                    if (Array.isArray(item?.['@graph'])) {
                        for (const sub of item['@graph']) {
                            if (sub?.['@type'])
                                types.push(String(sub['@type']).toLowerCase());
                        }
                    }
                }
            }
     ...
     */
    static isParkedOrThin(audit) {
        if (audit.crawlResult?.isParkedOrHolding)
            return true;
        const pages = audit.crawlResult?.pages || audit.pages || [];
        if (pages.length === 0)
            return true;
     ...
     */
    static findDefaultBusinessType(industryName) {
        const match = industryTaxonomy_js_1.INDUSTRY_TAXONOMY.find((ind) => ind.name.toLowerCase() === industryName.toLowerCase() || ind.id.toLowerCase() === industryName.toLowerCase());
...
exports.IndustryClassifier = IndustryClassifier;
